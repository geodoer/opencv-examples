...
void process(const std::string& dir, const std::string& out_dir)
{
        ...
        ++i)
    {
        ...
        double residual;
        Matx44d pose;
        icp.registerModelToScene(templateModel, model, residual, pose);

        if (residual > 1)
        {
            ++errorCnt;
            std::cout << "[Error] " << residual << " " << file << std::endl;
            continue; //不输出，计算失败
        }

        auto path = out_dir + "/" + fileName;
        Mat result_LocalAToB = transformPCPose(templateModel, pose);
...
{
    std::string in_dir = DATA_PATH "models/1_0.05";
    std::string out_dir = in_dir + "_icp";
